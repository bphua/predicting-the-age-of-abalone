...
The data contains no missing values. Two outliers were noticed for Height and removed. We also removed any instances where the sum of the constituent weights was larger than the Whole_weight. 
...
data <- data %>% filter(Length, Diameter, Height, `Whole_weight`, `Shucked_weight`, `Viscera_weight`, `Shell_weight` != 0) %>% filter(`Shucked_weight` + `Viscera_weight` + `Shell_weight` > `Whole_weight`)
summary(data)
...
varnorm = autoplot(step.back.aic,which=1:2) + theme_classic()
ggsave("report/varnorm.png", varnorm, dpi=600)
```
...
## Final Analysis:
The first step is to determine an appropriate model for multiple regression of the data. We use the AIC model selection method in the backwards direction to achieve this. We begin with the full model and remove the least informative variable after every iteration. We also undergo a log transformation of the dependent variable rings to ensure normality and homoscedasticity assumptions hold for regression. 
```{r}
# Now we take the log of ring values and add it to the original dataset
data = data %>% mutate(Rings = log(Rings))
